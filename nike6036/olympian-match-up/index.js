(() => {
    ...
    "node_modules/lodash/lodash.js"(exports, module) {
      ...
      (function() {
        ...
        var runInContext = function runInContext2(context3) {
          context3 = context3 == null ? root : _5.defaults(root.Object(), context3, _5.pick(root, contextProps));
          ...
          var DataView2 = getNative(context3, "DataView"), Map2 = getNative(context3, "Map"), Promise2 = getNative(context3, "Promise"), Set2 = getNative(context3, "Set"), WeakMap = getNative(context3, "WeakMap"), nativeCreate = getNative(Object2, "create");
    ...
    "node_modules/howler/dist/howler.js"(exports) {
      ...
      (function() {
        ...
        var setupAudioContext = function() {
          ...
          var version6 = appVersion ? parseInt(appVersion[1], 10) : null;
          if (iOS && version6 && version6 < 9) {
  ...
  var convertCell = (type, value) => {
    if (type.charAt(0) === "_") {
      ...
      return toArray3(value, dataType);
    }
    switch (type) {
  ...
  var fetchWithAuth = (supabaseKey, getAccessToken, customFetch) => {
    ...
    return (input, init4) => __awaiter6(void 0, void 0, void 0, function* () {
      var _a;
      ...
      return fetch3(input, Object.assign(Object.assign({}, init4), { headers }));
    });
  };
  ...
  var CARD_BACK = "https://uploads-ssl.webflow.com/66671214a665d73716d2ba1c/666766750422afad2a77e6c0_card-back.jpg";
  ...
  var Timer2 = class {
    constructor(timerElId) {
      this.timerElId = timerElId;
      this.startTime = null;
    ...
    update() {
      ...
      const timeEl = document.querySelector(this.timerElId);
      const timeDiff = Math.round((now - this.startTime) / 1e3);
      this.minutesUsed = Math.floor(timeDiff / 60);
      this.secondsUsed = timeDiff % 60;
      const minutesStr = this.minutesUsed;
      const secondsStr = import_lodash.default.padStart(Math.abs(this.secondsUsed), 2, "0");
      const timeString = `${minutesStr}:${secondsStr}`;
      timeEl.innerHTML = timeString;
    }
    getTimeElapsed() {
      return this.minutesUsed * 60 + this.secondsUsed;
    }
  };
  var timer_default = Timer2;

  // src/helpers/game-cards.js
  init_live_reload();
  function fadeInGameCards(cb) {
    gsapWithCSS.to(
      ".mu-game-card",
      { opacity: 1, duration: 0.15, ease: "power1.inOut", stagger: 0.05, onComplete: cb }
    );
  }
  function fadeOutGameCards(cb) {
    gsapWithCSS.to(
      ".mu-game-card",
      { opacity: 0, duration: 0.15, ease: "power1.inOut", stagger: 0.05, onComplete: cb }
    );
  }

  // src/classes/sound-manager.js
  init_live_reload();
  var import_howler = __toESM(require_howler(), 1);
  var SoundManager = class {
    constructor() {
      this.soundUrls = {
        collect: "https://cdn.jsdelivr.net/gh/wearevictory/static-files@main/internal/sound_effects/fruit-collect.wav",
        complete: "https://cdn.jsdelivr.net/gh/wearevictory/static-files@main/internal/sound_effects/big-egg-collect.wav"
      };
      this.sounds = {
        collect: new import_howler.Howl({ src: this.soundUrls.collect }),
        complete: new import_howler.Howl({ src: this.soundUrls.complete })
      };
    }
    collect(play = true) {
      if (play)
  ...
  function initSearchActions(data, cb) {
    ...
    const debouncedSearch = import_lodash3.default.debounce(handleSearch, 300);
  ...
  var Game = class {
    constructor() {
      ...
      this.timer = new timer_default("#time");
      this.level = 1;
    }
    setPlayerInfo(playerInfo) {
      this.playerInfo.email = playerInfo.email;
      this.playerInfo.fullName = playerInfo.fullName;
      this.playerInfo.team = playerInfo.team;
      this.playerInfo.location = playerInfo.location;
    }
    async init() {
      await this.fetchAthletes(false);
      await this.initCards();
    }
    async fetchAthletes(isSeasoned = false) {
      const { data } = await supabase_default.from("athletes").select("*").eq("is_seasoned_athlete", isSeasoned);
      this.athletes = data;
    }
    async initCards() {
      this.cards = import_lodash4.default.shuffle([...this.athletes, ...this.athletes]).map((athlete, index) => ({
        id: athlete.id,
        index,
        flipped: false,
        image: import_lodash4.default.get(athlete, "image_url", "")
      }));
    }
    pickCard(cardIndex) {
      if (this.boardIsLocked)
        return;
      if (this.pickedCards[0] && cardIndex === this.pickedCards[0].index)
        return;
      if (!this.started) {
        this.started = true;
        this.timer.start();
      }
      const selectedCard = this.cards.filter((card) => card.index === cardIndex)[0];
      this.pickedCards.push(selectedCard);
      this.increaseFlips();
      this.updateFlipsUI();
    }
    checkForMatch() {
      return this.pickedCards.length === 2 && this.pickedCards[0].id === this.pickedCards[1].id;
    }
    addFlippedCard(card) {
    ...
    increaseFoundPairs() {
      if (this.level === 1) {
        this.foundPairs.level1++;
      } else {
        this.foundPairs.level2++;
      }
    }
    async upgradeLevel() {
      if (this.level === 1) {
        this.level = 2;
        await supabase_default.from("face_matching_players").update({ levels_completed: 1 }).eq("email", this.playerInfo.email);
        await this.fetchAthletes(true);
        await this.initCards();
      }
    }
    async end() {
      this.isOver = true;
      this.timeCompleted.level2 = this.timer.getTimeElapsed();
      sound_manager_default.complete();
      const { data } = await supabase_default.from("face_matching_players").select().eq("email", this.playerInfo.email);
      if (data && data.length > 0) {
        await supabase_default.from("face_matching_players").update({
          full_name: this.playerInfo.fullName,
          team: this.playerInfo.team,
          location: this.playerInfo.location,
          time_completed_1: this.timeCompleted.level1,
          time_completed_2: this.timeCompleted.level2,
          flips_1: this.flips.level1,
          flips_2: this.flips.level2
        }).eq("email", this.playerInfo.email);
      } else {
        await supabase_default.from("face_matching_players").insert({
          full_name: this.playerInfo.fullName,
     ...
     * **********************************
     * **********************************
     * Functions that are bound to the UI
     * **********************************
     */
    initCardsUI(wrapperEl, card) {
      const cardEl = document.createElement("div");
      cardEl.classList.add("mu-game-card");
      const frontFaceEl = document.createElement("img");
      frontFaceEl.classList.add("front-face");
      frontFaceEl.setAttribute("src", card.image);
      const backfaceEl = document.createElement("img");
      backfaceEl.classList.add("back-face");
      backfaceEl.setAttribute("src", CARD_BACK);
      cardEl.appendChild(frontFaceEl);
      cardEl.appendChild(backfaceEl);
      wrapperEl.appendChild(cardEl);
    }
    async checkGameEnding(gameCardsWrapperEl) {
      if (this.level === 1 && this.foundPairs.level1 === this.cards.length / 2) {
        fadeOutGameCards(() => this.completeLevel1(gameCardsWrapperEl));
      } else if (this.level === 2 && this.foundPairs.level2 === this.cards.length / 2) {
        await this.end();
        const totalTime = document.querySelector("#total-time");
        const totalFlips = document.querySelector("#total-flips");
        const secondsUsed = this.timeCompleted.level1 + this.timeCompleted.level2;
        const minutes = parseInt(secondsUsed / 60);
        const seconds = secondsUsed - minutes * 60;
        const timeString = `${minutes}:${import_lodash4.default.padStart(seconds, 2, "0")}`;
        totalTime.innerHTML = timeString;
        totalFlips.innerHTML = this.flips.level1 + this.flips.level2;
        this.openLeaderboard();
      }
    }
    initListeners(gameCardsWrapperEl) {
      gameCardsWrapperEl.childNodes.forEach((gameCardEl, index) => {
        gameCardEl.addEventListener("click", async () => {
          if (this.boardIsLocked)
            return;
          this.pickCard(index);
          gameCardEl.classList.add("flip");
          if (this.flippedCards.length === this.cards.length - 2)
            await wait(1e3);
          if (this.pickedCards.length < 2)
            return;
          if (this.checkForMatch()) {
            gameCardsWrapperEl.childNodes[this.pickedCards[0].index].style.pointerEvents = "none";
            gameCardsWrapperEl.childNodes[this.pickedCards[1].index].style.pointerEvents = "none";
            this.addFlippedCard(this.pickedCards[0]);
            this.addFlippedCard(this.pickedCards[1]);
            this.resetPickedCards();
            this.increaseFoundPairs();
            this.unlockBoard();
            this.checkGameEnding(gameCardsWrapperEl);
            sound_manager_default.collect();
          } else {
            this.lockBoard();
            setTimeout(() => {
              gameCardsWrapperEl.childNodes[this.pickedCards[0].index].classList.remove("flip");
              gameCardsWrapperEl.childNodes[this.pickedCards[1].index].classList.remove("flip");
              this.unlockBoard();
              this.resetPickedCards();
            }, 800);
          }
    ...
    updateFlipsUI() {
      const flipsCounterEl = document.querySelector("#flips");
      if (this.level === 1) {
        flipsCounterEl.innerHTML = this.flips.level1;
      } else {
        flipsCounterEl.innerHTML = this.flips.level2;
      }
    }
    updateTimer() {
      ...
      const timeDiffSec = Math.round((/* @__PURE__ */ new Date() - this.startTime) / 1e3);
      this.minutesUsed = Math.floor(timeDiffSec / 60);
      this.secondsUsed = timeDiffSec % 60;
      const minutesStr = this.minutesUsed;
      const secondsStr = import_lodash4.default.padStart(Math.abs(this.secondsUsed), 2, "0");
      const timeString = `${minutesStr}:${secondsStr}`;
      timeEl.innerHTML = timeString;
    }
    async completeLevel1(gameCardsWrapperEl) {
      const level1Congrats = document.querySelector("#level-1-congrats");
      const continueLevel2 = document.querySelector("#continue-level-2");
      this.timer.pause();
      sound_manager_default.complete();
      await this.upgradeLevel();
      level1Congrats.classList.remove("hidden");
      gameCardsWrapperEl.innerHTML = "";
      gsapWithCSS.to(
        ".mu-game-card",
        { opacity: 0, duration: 0.25, ease: "power1.inOut", stagger: 0.1 }
      );
      this.cards.forEach((card) => this.initCardsUI(gameCardsWrapperEl, card));
      this.updateFlipsUI();
      this.resetFlippedCards();
      this.resetPickedCards();
      this.unlockBoard();
      this.initListeners(gameCardsWrapperEl);
      this.timeCompleted.level1 = this.timer.getTimeElapsed();
      this.timer.reset(false);
      continueLevel2.addEventListener("click", async () => {
        const levelEl = document.querySelector("#level");
        levelEl.innerHTML = `Stage ${this.level}`;
        levelEl.classList.add("orange");
        level1Congrats.classList.add("hidden");
  ...
  window.Webflow.push(
    async () => {
      ...
      const pageHero = document.querySelector(".mu-hero");
      const pageHeroLeftImg = pageHero.querySelector(".mu-hero-img.left");
      const pageHeroLogo = pageHero.querySelector(".mu-hero-logo");
      const pageHeroRightImg = pageHero.querySelector(".mu-hero-img.right");
      gsapWithCSS.set(document.body, { overflowY: "clip" });
      const tl = gsapWithCSS.timeline({
        onComplete: () => gsapWithCSS.set(document.body, { overflowY: "scroll" })
      });
      tl.to(
        [pageHero, mainNav],
        { opacity: 1, duration: 0.2, ease: "power1.inOut" }
      ).to(
        [pageHeroLeftImg, pageHeroRightImg, pageHeroLogo],
        { opacity: 1, duration: 0.2, ease: "power1.inOut", stagger: 0.1 },
        "+=0.5"
      ).to(
        pageContent,
        { opacity: 1, duration: 0.3, ease: "power1.inOut" }
      );
      const gameCardsWrapperEl = document.querySelector(".mu-game-cards");
      await game_default.init();
      game_default.cards.forEach((card) => game_default.initCardsUI(gameCardsWrapperEl, card));
      game_default.initListeners(gameCardsWrapperEl);
    }
  );
  window.Webflow.push(
    () => {
  ...
  window.Webflow.push(() => {
    ...
    const playerInfoForm = document.querySelector("#mu-form");
    playerInfoForm.addEventListener("submit", async (e) => {
      ...
      const locationOther = document.querySelector('input[name="location_other"]').value;
      game_default.setPlayerInfo({
        email,
        fullName,
        team,
        location: location2 === "other" ? locationOther : location2
      });
